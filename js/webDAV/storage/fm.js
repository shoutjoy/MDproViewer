...
const FM = (() => {
        ...
        function renderNode(node, depth, container) {
            ...
            Object.keys(node.children).sort().forEach(folderName => {
                ...
                hdr.innerHTML =
                    ...
                    `<button class="folder-move-btn" title="폴더 이동" data-path="${_esc(folderPath)}" ` +
                    `onclick="event.stopPropagation();FM.moveFolder(this)">↗</button>` +
                    `<button class="folder-rename-btn" title="폴더명 고치기" data-path="${_esc(folderPath)}" ` +
            ...
            node.files.sort((a, b) => (b.modified||0) - (a.modified||0)).forEach(f => {
                ...
                row.innerHTML =
                    ...
                    `<button class="file-rename-btn" title="파일명 고치기" onclick="event.stopPropagation();FM.renameFile(this)">✎</button>` +
    ...
    /** 로컬 파일 트리에서 저장된 경로 행을 활성으로 표시하고 목록을 다시 그린다 */
    ...
    /* ── PULL: GitHub → 로컬 캐시 ────────────────────────
       ...
       7. 이미 열린 탭에 "갱신됨" 알림                   */
    /** 설정된 GitHub(mdliveData) 저장소를 ZIP으로 다운로드한다. 새 탭에서 GitHub ZIP URL을 연다 */
    function downloadGitHubZip() {
        ...
        if (typeof App !== 'undefined' && App._toast) App._toast('📦 ZIP 다운로드 시작');
    }
    ...
    async function syncToGitHub() {
        ...
        try {
            ...
            if (hasRemote) {
                _syncStatus('ing', '⟳ 원격 변경 있음 — Pull 먼저 실행 중…');
                await pullFromGitHub();
                await _loadBaseSHAs();
                const remoteSHAs2 = await GH.getRemoteSHAs();
                const res2 = await _classifyFiles(remoteSHAs2);
                if (res2.hasConflict) {
                    const names = res2.files.filter(f => f.status === 'conflict').map(f => `  🔴 ${f.name}`).join('\n');
                    _syncStatus('err', '✗ 충돌 — 해결 후 Push 하세요');
                    alert(`Push 차단: Pull 후 충돌 파일이 있습니다.\n${names}\n\n충돌을 해결한 뒤 다시 Push 하세요.`);
                    _setBusy(false);
                    return;
                }
                if (res2.hasRemote) {
                    _syncStatus('err', '✗ 원격과 내용이 다릅니다');
                    alert('원격과 내용이 다릅니다. Pull 후 로컬에서 병합한 뒤 다시 Push 하세요.');
                    _setBusy(false);
                    return;
                }
                files.length = 0;
    ...
    /** inDB 파일을 지정된 로컬 폴더에 저장. 폴더 미선택 시 안내, 파일 선택 모달 표시 */
    async function importFromInDB() {
        if (!dirHandle) { alert('먼저 폴더를 선택하세요.'); return; }
        if (typeof InDB === 'undefined') { alert('inDB를 사용할 수 없습니다.'); return; }
        const files = InDB.getFiles();
        if (!files || !files.length) { alert('inDB에 가져올 파일이 없습니다.'); return; }

        /* 저장 폴더 옵션: 루트 + _subHandles, _emptyFolders */
        const folderOptions = [{ label: '📁 (루트)', value: '' }];
        ...
        const targetHandle = folderVal
            ...
            : dirHandle;
        if (!targetHandle) { alert('폴더 핸들을 찾을 수 없습니다.'); return; }
        ...
        try {
            ...
            let ok = 0, fail = 0;
            let applyToAllChoice = null;  /* { action, filename?, saveasCounter } — 나머지에도 같은 행동 적용 */
            for (const path of selectedPaths) {
                ...
                /* 로컬에 이미 있으면 덮어쓰기/다른이름저장/건너뛰기 선택 */
                try {
                ...
                try {
                    ...
                    fail++;
                    console.warn('importFromInDB 실패:', path, e);
                }
            }
            _subHandles = {};
            _emptyFolders = {};
            await _syncFromHandle();
            if (typeof App !== 'undefined' && App._toast) App._toast(`📥 ${ok}개 가져옴${fail ? ` (${fail}개 실패)` : ''}`);
        } catch (e) {
            if (e.name === 'NotAllowedError') {
                if (confirm('쓰기 권한이 필요합니다. 폴더를 다시 선택하시겠습니까?')) selectFolder();
            } else { alert('가져오기 실패: ' + (e.message || e)); }
        }
    }

    /** 경로로 하위 디렉터리 핸들 획득. 없으면 생성 */
    ...
    /** inDB content를 파일 쓰기용 데이터로 변환. base64 이미지는 디코드 */
    ...
    /** 로컬에 파일이 있을 때 덮어쓰기/다른이름저장/건너뛰기 선택 모달. 반환: { action, applyToAll?, filename? } 또는 null */
        ...
        return new Promise(resolve => {
            ...
            box.innerHTML = `
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
                    <span style="font-size:14px;font-weight:700;color:var(--txh)">⚠ 파일이 이미 존재합니다</span>
                ...
                <div style="margin-bottom:14px;display:none" id="fm-ow-saveas-wrap">
                    <label style="font-size:11px;color:var(--tx3);display:block;margin-bottom:4px">다른 파일 이름</label>
                    <input id="fm-ow-newfile" type="text" value="" placeholder="새 파일명 입력"
                ...
                <label style="display:flex;align-items:center;gap:6px;margin-bottom:14px;cursor:pointer;font-size:11px;color:var(--tx2)">
                    ...
                    <span>나머지 충돌 파일에도 같은 행동 적용</span>
                </label>
                <div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap">
                    <button id="fm-ow-skip" style="padding:6px 14px;border-radius:6px;border:1px solid var(--bd);background:var(--bg3);color:var(--tx2);font-size:12px;cursor:pointer">건너뛰기</button>
                    <button id="fm-ow-saveas" style="padding:6px 14px;border-radius:6px;border:1px solid var(--bd);background:var(--bg3);color:var(--tx2);font-size:12px;cursor:pointer">다른 이름으로</button>
                    <button id="fm-ow-overwrite" style="padding:6px 18px;border-radius:6px;border:none;background:var(--ac);color:#fff;font-size:12px;font-weight:600;cursor:pointer">덮어쓰기</button>
                </div>`;
            ...
            document.getElementById('fm-ow-saveas').onclick = () => {
                if (saveasWrap.style.display === 'block') {
                    ...
                    newfileInput.focus();
                    newfileInput.select();
                    document.getElementById('fm-ow-saveas').textContent = '✔ 저장';
                }
            };
        ...
        return new Promise(resolve => {
            ...
            box.innerHTML = `
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-shrink:0">
                    <span style="font-size:14px;font-weight:700;color:var(--txh)">📥 inDB에서 가져오기</span>
                ...
                <div style="margin-bottom:10px;flex-shrink:0">
                    <label style="font-size:11px;color:var(--tx3);display:block;margin-bottom:4px">저장 폴더</label>
                ...
                <div style="margin-bottom:8px;flex-shrink:0">
                    <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:11px;color:var(--tx3)">
                        <input type="checkbox" id="fm-imp-all"> 전체 선택
                    </label>
                </div>
                ...
                <div style="display:flex;gap:8px;justify-content:flex-end;flex-shrink:0">
                    <button id="fm-imp-cancel" style="padding:6px 16px;border-radius:6px;border:1px solid var(--bd);background:var(--bg3);color:var(--tx2);font-size:12px;cursor:pointer">취소</button>
                    <button id="fm-imp-ok" style="padding:6px 18px;border-radius:6px;border:none;background:var(--ac);color:#fff;font-size:12px;font-weight:600;cursor:pointer">✔ 가져오기</button>
                </div>`;
    ...
    /* 로컬: 디렉터리 트리를 재귀 복사한다(최대 depth, 바이너리 안전) */
    ...
    /** 로컬: dirHandle 기준으로 폴더를 다른 논리 경로로 옮기거나 이름만 바꾼다(move API 우선, 실패 시 복사+삭제) */
    ...
    /** 열린 탭의 filePath에서 폴더 접두어를 일괄 치환한다 */
    ...
    async function renameFile(btn) {
        ...
        try {
            ...
            _emptyFolders = {};
            await _syncFromHandle();
            App._toast(`✅ 파일명 변경: ${newName}`);
        } catch (e) {
            alert('파일명 변경 실패: ' + (e.message || e));
        }
    }
    ...
    async function renameFolder(btn) {
        ...
        try {
            ...
            _emptyFolders = {};
            await _syncFromHandle();
            App._toast(`✅ 폴더명 변경: ${newLeaf}`);
        } catch (e) {
            alert('폴더명 변경 실패: ' + (e.message || e));
        }
    }

    /* ── 로컬 폴더 이동 (상위 폴더만 변경, 이름 유지) ─── */
    ...
    /** 이동 대상 폴더 선택 모달 (파일·폴더 공통, title 지정 가능) */
    ...
    /** .mdp에서 복원: files 배열을 IDB에 저장하고 UI 갱신 (캐시 모드) */
