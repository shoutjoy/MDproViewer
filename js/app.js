...
/** ???????거?????????????븍툖??????????몃뒇????????????????덉떻?????????嶺뚮씚維?????????????嫄????????????????鶯ㅺ동???猿듽렂??????SHA-256 ??????????롮쾸?椰????(??????????????????????????????????⑤슢堉??거?????? ????????關?쒎첎?嫄???? */
...
// initDB ??onload ????????????????????????븐뼐???????????????????????썹땟戮녹??諭?????⑸㎦???????????????????饔낅떽???????????????????????? ???????耀붾굝????????
if (editorTextarea) {
...
/** NotebookLM ????? ????????????????????????怨뺤떪??????????(onload ????????????밸븶筌믩끃??獄???????멥렑?????????嶺뚮ㅎ????? */
let pendingExternalContent = null;
let receivedExternalContent = false;
/** NotebookLM?????????????쏙쭗??????????거?????????????????????됰Ŧ?????????????????????밸븶筌믩끃??獄???????멥렑??????????????????援쏂굜??????????????怨뺤떪????????'=' ??????????????雅?퍔瑗?땟?????????????繹먮끍???'-'????????轅붽틓????곌램伊??????*/
...
window.addEventListener('message', function (ev) {
    ...
    // 1?? scholarToMDPaste ?????????거??????????????postMessage ????????????????????????readClipboard/useClipboard)
    ...
    const openerOk = window.opener && ev.source === window.opener; // ???????????源녾텛??????????????????????????
    ...
    if (typeof loadFromExternalContent === 'function') {
        ...
        if (typeof showToast === 'function') showToast("????????????????癲ル슢???????????????癲????????????????????????????ㅻ쑋雍?");
    }
});
...
window.onload = async () => {
    try {
        ...
        // ?????????됰Ŧ???????????????????????????怨뺤떪????遺얘턁??????????????꾩룆梨띰쭕??????????嫄??????????????嫄????????????? ????????關?쒎첎?嫄????(NotebookLM ????? ??????????????)
        ...
        // ??? ??????????????????븐뼐????????????????????? onload?????????怨뺤떪?????????????⑥レ뿥?????轅붽틓????? ????textarea??????????????????????????嶺뚮씚維???????????????????
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.altKey && !e.ctrlKey && !isAltGraph && (e.code === 'Digit4' || e.key === '4')) {
            e.preventDefault();
            toggleTheme();
            showToast("????????????쏙쭗????????롮쾸?椰?癲ル슢?®쳥?????????????쎛 ?????????怨뺤떪????????嶺뚮씚維??????????????????????????????ㅻ쑋雍?");
        }
    ...
    window.addEventListener('beforeunload', closePreviewPopupWindow);
        ...
        console.error('???????饔낅떽?????????????????????筌??', e);
        if (typeof showToast === 'function') showToast('???????饔낅떽???????????????????????筌???????????? ???????ш끽維뽳쭩?뱀땡???얩맪?????????????????饔낅떽?????????????????????????????ㅻ쑋雍? ?????????썹땟戮녹??諭?????⑸㎦???????????거??????????????????됰Ŧ?????????????????⑤뜪????');
    }
};
...
function preprocessFootnotesForView(raw) {
    ...
    const items = defs.map(function (d) {
        const content = (d.content || 'Footnote.')
            .replace(/^<span\b[^>]*>/i, '')
            .replace(/<\/span>\s*$/i, '')
            .replace(/\s*<a class="md-footnote-backref"[^>]*>[\s\S]*?<\/a>\s*$/i, '')
            .trim() || 'Footnote.';
        return '<li id="md-footnote-' + d.id + '">' + content + ' <a class="md-footnote-backref" href="#md-footnote-ref-' + d.id + '">??/a></li>';
    }).join('\n');
...
function getPreviewPopupDocumentHtml() {
    return '<!doctype html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>MDproViewer Preview</title><style>'
        ...
        + '<button type=\"button\" onclick=\"window.opener&&window.opener.previewPopupAdjustScale(0.1)\">?????????</button>'
        + '<button type=\"button\" onclick=\"window.opener&&window.opener.previewPopupAdjustFontSize(-1)\">??????????饔낅떽??????</button>'
...
function openPreviewPopupWindow() {
    ...
    if (!previewPopupWindow) {
        showToast('????????袁⑸즴筌?씛彛???돗????????????????????嫄??????????怨뺤른??????????????筌띿솘?????????????? ????????袁⑸즴筌?씛彛???돗????????????????????????????????源낆┰?????????????????숈춻??????????됰Ŧ???????????????됰Ŧ?????????????????⑤뜪????');
        return;
    }
    ...
    try {
        ...
        showToast('???????????亦낃콛?????釉먯뒠????????????????饔낅떽???????????????????????筌???????????? ???????ш끽維뽳쭩?뱀땡???얩맪?????????????????饔낅떽?????????????????????????????ㅻ쑋雍?');
        return;
    }
...
function toggleMode(mode) {
    ...
    if (!vc || !ec) {
        console.warn('toggleMode: viewer-container ?????content-viewport????????嫄??????????????????????롮쾸?椰????????????????ㅻ쑋雍?', { vc: !!vc, ec: !!ec });
        return;
    }
...
async function restoreFromMpv(data) {
    ...
    renderDBList();
    showToast("???????ш끽維뽳쭩?뱀땡???얩맪????????????????????????????????癲ル슢?????????????怨뺤떪????遺얘턁?????????????????????????ㅻ쑋雍?");
}
...
function renderMergeList() {
    ...
    if (mergeListState.length === 0) {
        listEl.innerHTML = '<p class="text-sm text-slate-500 dark:text-slate-400 py-4 text-center">??????????ROOT)???????????????????썹땟戮녹??諭?????⑸㎦? ??????????롮쾸?椰????????????????ㅻ쑋雍?</p>';
        return;
    }
    ...
    if (filtered.length === 0) {
        listEl.innerHTML = `<p class="text-sm text-slate-500 dark:text-slate-400 py-4 text-center">${mergeListSelectedOnly ? '?????????????????????????????????썹땟戮녹??諭?????⑸㎦? ??????????롮쾸?椰????????????????ㅻ쑋雍?' : '?????嶺뚮씚維??????????????嶺뚮씚維??????????????븐뼐???????? ??????????롮쾸?椰????????????????ㅻ쑋雍?'}</p>`;
        lucide.createIcons();
        return;
    }
    listEl.innerHTML = filtered.map(({ item, idx }) => `
        <div class="flex items-center gap-2 p-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600" data-idx="${idx}">
            ...
            <div class="flex flex-col shrink-0">
                <button type="button" onclick="moveMergeItem(${idx},-1)" class="p-0.5 text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400" title="??????????밸븶筌믩끃??獄???????멥렑??????>??/button>
                <button type="button" onclick="moveMergeItem(${idx},1)" class="p-0.5 text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400" title="??????????밸븶筌믩끃??獄???????멥렑?????>??/button>
            </div>
        </div>
    `).join('');
...
async function exportMpv() {
    ...
    URL.revokeObjectURL(url);
    closeBackupModal();
    showToast("MPV(JSON)???????????뀀맩鍮??????嶺???????????? ???????거???????????? .json???????????????ш끽維뽳쭩?뱀땡???얩맪???????????????????거????????遺얘턁????????熬곣뫖???????????됰Ŧ??????????");
}
...
async function saveFileAs() {
    ...
    if (!result || result.canceled) return;
    if (result.error) {
        showToast(`???????????源녾텛????????????????????????????????????산뭐????????????????????ㅻ쑋雍? ${result.error}`);
        return;
    }
    setCurrentDocumentInfo(result.fileName, result.filePath);
    showToast("?????????????????源녾텛????????????????????????뀀맩鍮??????嶺????????????");
...
function renderTOC() {
    ...
    if (isSidebarCollapsed) {
        tocList.innerHTML = `<div class="p-2 text-center text-xs text-slate-400">??????嫄??????????????嫄????????????????곗뿨????????????????????????⑤뜪???/div>`;
        return;
    }
    ...
    if (!found) {
        tocHtml = '<div class="p-4 text-xs text-slate-400 text-center">?????????????嫄??????????????嫄????????????????곗뿨?????룸㈇媛?????縕??力?肉???⑤슢堉??⑤슣瑗?? ??????????롮쾸?椰????????????????ㅻ쑋雍?<br>(# ???????됰Ŧ????????????룰퀬???傭?끆???嶺?壤???</div>';
    }
...
function saveToDB() {
    ...
    currentActionCallback = (title) => {
        ...
        tx.oncomplete = () => {
            showToast("?????????????????썹땟戮녹??諭?????⑸㎦? inDB???????????⑤슦????????????????怨룐솲????????????????????ㅻ쑋雍?");
            renderDBList();
            if (isSidebarHidden) toggleSidebarVisibility();
        };
    };
            ...
            docItem.innerHTML = `
                <div class="flex flex-col gap-1 doc-item-inner">
                    ...
                    <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity doc-action-btns">
                        <button onclick="event.stopPropagation(); loadFromDB('${doc.id}')" class="text-[10px] bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded border border-indigo-100 dark:border-indigo-800 font-bold hover:bg-indigo-600 hover:text-white">???????????癲???????????????/button>
                        <button onclick="event.stopPropagation(); openMoveModal('${doc.id}')" class="text-[10px] bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-600 font-bold hover:bg-slate-200 dark:hover:bg-slate-600">?????/button>
...
function applyScholarPaste(content) {
    ...
    renderTOC();
    performAutoSave();
    if (typeof showToast === 'function') showToast("???????????癲??????????ш끽維뽳쭩?뱀땡???얩맪??????????????룸챷援???????????");
}

/** 2?? ??????????밸븶筌믩끃??獄???????멥렑?????????content script??????????? window.acceptScholarPaste(content, notebookLm) ???????거?????????(notebookLm===false ?????'=' ??????轅붽틓????곌램伊???????????? */
...
/** URL ?????postMessage????????????밸븶筌믩끃??獄???????멥렑??????? ?????????????ш끽維뽳쭩?뱀땡???얩맪???????????????????????鶯ㅺ동??????(?????????怨뺤떪????遺얘턁??????????????꾩룆梨띰쭕??????????嫄??????????????嫄????????????????癲?? */
...
/** URL ???????????content/title ???????거?????????????????????????鶯ㅺ동?????? ???????????鶯ㅺ동????????????????????黎앸럽??筌?????怨쀫엥?????true */
...
async function insertUserInfoAtCursor() {
    if (!isEditMode) {
        showToast('???????거?????????????諛몃마嶺뚮????????????????嫄??????????????嫄???????????????????됰Ŧ?????????????????⑤뜪????');
        return;
    }
    if (!db) {
        showToast('??????????????????????癲??????????븐뼐??????????????????寃몃탿????????????????????????????ㅻ쑋雍?');
        return;
    }
    ...
    if (String(u.contact || '').trim()) lines.push('???????????ㅻ깹?????? ' + String(u.contact).trim());
...
function insertHtmlImageAtCursor(imageUrl, altText) {
    ...
    performAutoSave();
    if (activeSidebarTab === 'toc') renderTOC();
    showToast('??????????嫄??????? HTML ??????????산뭐???????嚥???癲??關?쒎첎??????????????????????????????ㅻ쑋雍?');
}
...
function openImageInsertExternalLink(type) {
    ...
    try {
        ...
        if (!win) {
            setImageInsertStatus('????????????????????????? ??????????????????????????⑤뜪??????????嫄??????????怨뺤른??????????????筌띿솘??????????????????????ㅻ쑋雍?', true);
            return;
        }
        setImageInsertStatus('Image pasted. Click [imgBB] Upload to continue.', false);
    } catch (e) {
        setImageInsertStatus('??????嫄??????????????곷♧??????怨뺣???援욂펺??쨨???룰퀬寃?????饔낅떽????????곕선???????? ??????嫄????????????????????', true);
    }
}
...
function readImageFileForInsertModal(file) {
    if (!file || String(file.type || '').indexOf('image') !== 0) {
        setImageInsertStatus('??????????嫄??????? ????????????繹먮굝????????????????????????????????????ㅻ쑋雍?', true);
        return;
    }
    const reader = new FileReader();
    reader.onload = function () {
...
function onImageInsertUploadDrop(event) {
    ...
    if (!file) {
        setImageInsertStatus('???????됰Ŧ???????????????????????????롮쾸?椰????????????????ㅻ쑋雍?', true);
        return;
    }
    readImageFileForInsertModal(file);
}
...
function cropImageInsertCurrent() {
    if (!imageInsertCurrentDataUrl) {
        setImageInsertStatus('Crop????????????嫄?????????????????⑥レ뿥?????轅붽틓????? ????????????????거?????????', true);
        return;
    }
    ...
    if (!imageInsertCropWindow) {
        setImageInsertStatus('Crop ??????嫄???????????????????????롮쾸?椰????????????????ㅻ쑋雍? ????????袁⑸즴筌?씛彛???돗???????????????????嫄??????????怨뺤른??????????????筌띿솘?????????????거??????????????????됰Ŧ?????????????????⑤뜪????', true);
        return;
    }
...
async function uploadImageInsertToImgbb() {
    if (!imageInsertCurrentDataUrl || imageInsertCurrentDataUrl.indexOf('data:image') !== 0) {
        setImageInsertStatus('???????????????????????嫄?????????????????⑥レ뿥?????轅붽틓????? ??????????寃몃탿?????遺얘턁??????????????????????????거?????????', true);
        return;
    }
    const apiKey = String(getImgbbApiKey() || '').trim();
    if (!apiKey) {
        setImageInsertStatus('imgBB API ??? ??????????롮쾸?椰????????????????ㅻ쑋雍? ?????????????????????sspimgAI??????????????????筌????????됰Ŧ?????????????????⑤뜪????', true);
        return;
    }
    setImageInsertStatus('Image pasted. Click [imgBB] Upload to continue.', false);
    try {
        ...
        setImageInsertStatus('imgBB ????????????????????筌?? ' + (e && e.message ? e.message : e), true);
    }
}
...
function insertImageFromModal(type) {
    ...
    if (!source) {
        setImageInsertStatus('???????URL ???????????????嫄?????????????????? ??????????롮쾸?椰????????????????ㅻ쑋雍?', true);
        return;
    }
...
function tidySeparatorSpacing(source) {
    ...
    // Setext underline(====) ??????癲ル슢?싩땟???????????影??젘???????1???????????? ???????????????????됰Ŧ????????????????????????????????    inFencedCodeBlock = false;
...
function convertSelectionPatternToTable() {
    ...
    if (!selectedText || !selectedText.trim()) {
        showToast('???????됰Ŧ??????????????거??????????????怨뺤떪?????????????????????????????????⑥レ뿥?????轅붽틓????? ????????????????????????ш끽維뽳쭩?뱀땡??????????');
        return;
    }
    ...
    if (maxCols < 2) {
        showToast('??????嫄???????????2?????????????癲????????????밸븶筌믩끃??獄???????멥렑????????됰Ŧ?????????? ?????????????????濚밸Ŧ援??????,/;/|)?????????거?????????????????????ш끽維뽳쭩?뱀땡??????????');
        return;
    }
...
function convertSelectionMarkdownToHtml() {
    ...
    if (typeof marked === 'undefined' || typeof marked.parse !== 'function') {
        showToast('Markdown parser????????嫄??????????????????????롮쾸?椰????????????????ㅻ쑋雍?');
        return;
    }
    ...
    const end = editorTextarea.selectionEnd;
    if (start === end) {
        showToast('HTML???????????怨뺤떪?????????????????????????????????⑥レ뿥?????轅붽틓????? ????????????????????????ш끽維뽳쭩?뱀땡??????????');
        return;
    }
...
/** Google AI Studio / Gemini API ?? ?????????怨뺤떪?????AIza?????????됰Ŧ???????????????? 39?????????*/
...
function openScholarSearchWindow(query) {
    ...
    if (!win) showToast('????????袁⑸즴筌?씛彛???돗????????????????????嫄??????????怨뺤른??????????????筌띿솘?????????????? ????????袁⑸즴筌?씛彛???돗????????????????????????????????源낆┰?????????????????숈춻??????????됰Ŧ???????????????됰Ŧ?????????????????⑤뜪????');
}
...
async function saveImgbbApiKeyFromModal() {
    ...
    if (feedback) feedback.textContent = value ? 'imgBB API ??? ?????????뀀맩鍮??????嶺????????????' : 'imgBB API ??? ??????????';
...
function syncImgbbApiKeyInputs(value) {
    ...
    if (settingsInput && settingsInput.value !== v) settingsinput.value = '';
    const sspInput = document.getElementById('ssp-imgbb-api-key');
    if (sspInput && sspInput.value !== v) sspinput.value = '';
}
...
function setAiPasswordVerifiedUI(state) {
    ...
    if (state === 'ok') {
        ...
        if (fb) {
            fb.textContent = '???????거?????????????븍툖??????????몃뒇?????????????밸븶筌믩끃??獄???????멥렑???????????';
...
async function saveAiPassword() {
    ...
    if (!pwd) {
        showToast("???????거?????????????븍툖??????????몃뒇????????????????덉떻??????????怨쀫엥????????????????????????筌????????됰Ŧ?????????????????⑤뜪????");
    ...
    showToast("???????거?????????????븍툖??????????몃뒇??????????? ScholarAI ?????sspimgAI??????????꾩룆梨띰쭕?뚢뵾??????????????????嫄?????????????????????????????됰Ŧ??????????");
    await applyAiFeatureVisibility();
}

function updateAiScholarSspimgAvailability(verified) {
    // Chrome: ???????거?????????????븍툖??????????몃뒇????????????嫄?????????toggleAiPasswordSection??getAiSettings ?????????썹땟戮녹??諭?????⑸㎦???????????????????????????밸븶筌믩끃??獄???????멥렑?????????嶺뚮ㅎ?????
    // verified=false?????????????????됰뼸???????????????????????ㅻ쑋????????????ш끽維뽳쭩?뱀땡???얩맪??????볧뀮???됀????????? (IndexedDB ????????????沅걔?????????????????嫄??????????怨뺤른??????????????筌띿솘????
    ...
    if (hint) {
        if (verified) {
            hint.textContent = '???????거?????????????븍툖??????????몃뒇?????????????밸븶筌믩끃??獄???????멥렑??????????? ?????AI???????????????????????됰Ŧ?????????????????⑤뜪????';
            hint.className = 'text-xs text-green-600 dark:text-green-400';
        } else {
            hint.textContent = '???????거?????????????븍툖??????????몃뒇????????????????덉떻??????????怨쀫엥???????????????????????⑤슢堉??거????????????????????ㅳ늾????ScholarAI???????꿔꺂?ｉ뜮?????pimgAI??????????????????????????????????????ㅻ쑋雍?';
...
async function saveAiUserInfo() {
    ...
    if (!db) {
        showToast('??????????????????????癲??????????븐뼐??????????????????寃몃탿????????????????????????????ㅻ쑋雍? ??????????????????????????숈춻??????????됰Ŧ???????????????됰Ŧ?????????????????⑤뜪????');
        return;
    }
    const userInfo = readAiUserInfoFromModal();
    if (!userInfo.name && !userInfo.id && !userInfo.major && !userInfo.contact && !userInfo.email) {
        if (fb) fb.textContent = '????????됰Ŋ?????????????????レ쑜??????붺몭??잞쭜???????????????????嫄???????????????????????????????筌????????됰Ŧ?????????????????⑤뜪????';
        showToast('????????????⑤슢堉??거????????????????거?????????????????????????筌????????됰Ŧ?????????????????⑤뜪????');
        return;
    }
    await setAiSettings({ userInfo });
    if (fb) fb.textContent = '?????????⑤슦????????????????怨룐솲????????????????????ㅻ쑋雍? ??????????????????????????????????????????????????????????????癲????????';
    showToast('????????????거???????????????濾곌쑴裕?????????????????????壤?????????????쎛 ?????????⑤슦????????????????怨룐솲????????????????????ㅻ쑋雍?');
}
...
async function applyAiFeatureVisibility() {
    ...
    // ??????嫄??????????????嫄????????? ???????????????黎앸럽??筌?????怨쀫엥?????IndexedDB ????(Chrome ????????????????????????????癲ル슢??蹂?㎟???DOM ??????嫄??????????????????????꿔꺂??????뻿??????????븐뼐?곭춯?竊?????????????????????????????癲??)
...
function setAiSidebarWrapVisible(w, isLoading) {
    ...
    if (inner) {
        ...
        if (isLoading && !inner.querySelector('#scholar-ai-sidebar') && !inner.querySelector('#ssp-ai-sidebar')) {
            inner.innerHTML = '<div class="flex items-center justify-center h-full text-slate-500 dark:text-slate-400 text-sm p-4">AI ???????????????????遺븍き????????..</div>';
        }
    }
}
    ...
    var t = setInterval(function () {
        ...
        if (n > 100) {
            ...
            injectSidebarAIHtml().then(function (ok) {
                if (tryCb()) return;
                showToast(ok === false ? 'AI ?????????????????????????癲????? ??????嫄???????????????????? http ???????됰Ŧ????????????????????????????????怨뺤떪????????' : 'AI ????????????????????????????筌???????????? ??????????????????ㅻ쑋雍?');
            });
        }
    }, 50);
}
...
function openScholarAIFromHeader() {
    getAiSettings().then(function (s) {
        if (!s || !s.verified) {
            showToast('???????????????????????????거?????????????븍툖??????????몃뒇????????????????덉떻????????????????????거?????????????븍툖??????????몃뒇?????????????밸븶筌믩끃??獄???????멥렑??????????????????????????????ㅻ쑋雍?');
            return;
        }
...
function viewerSSPCropFromPanel() {
    ...
    if (!src) {
        showToast('????????⑥レ뿥?????轅붽틓????? ??????????嫄?????????????????袁⑸즴筌?씛彛???돗??????????????????????????????????거?????????');
        return;
    }
...
function ensureSidebarAILoaded() {
    ...
    window.SidebarAIConfig = {
        ...
        callbacks: {
            ...
            /**
             * SSP ??????????嫄??????? ??????????밸븶筌믩끃??獄???????멥렑?? Gemini ??????????嫄??????? ??????嫄??????????????嫄?????? :generateContent, Imagen?? :predict (generateImages ??????????????????黎앸럽??筌??沃섏쥓爾녜툣????
             */
            generateImage: async function (prompt, options) {
                const key = localStorage.getItem('ss_gemini_api_key') || '';
                if (!key || !String(key).trim()) throw new Error('API ??? ??????????롮쾸?椰????????????????ㅻ쑋雍? ????????????????????Gemini API ??? ????????????⑤슢堉??거??????????거?????????');
                let modelId = (options && options.modelId) || 'gemini-2.5-flash-image';
                const aspectRatio = (options && options.aspectRatio) || '1:1';
                /** false(????????=?????????????쇨덫櫻????????????嫄???????, true(??????嫄????????????=???????????????嫄????????????????븐뼐?????????????????????癲??*/
                ...
                if (!cand) throw new Error('??????????????????밸븶筌믩끃??獄???????멥렑???????????????????????쎛 ??????????롮쾸?椰????????????????ㅻ쑋雍? ??????嫄??????????????嫄?????????????????밸븶筌믩끃??獄???????멥렑????????????????????ル????????????거??????????????????됰Ŧ?????????????????⑤뜪????');
                ...
                if (cand.finishReason && cand.finishReason !== 'STOP') throw new Error('????????袁⑸즴筌?씛彛???돗?????????????????寃몃탿???????????????? ' + cand.finishReason);
                throw new Error('??????????嫄??????? ?????????? ??????????????????롮쾸?椰????????????????ㅻ쑋雍? ???????????源녾텛?????????????嫄??????? ??????嫄??????????????嫄???????????????????????????????怨뺤떪????????');
            },
...
async function loadAiSettingsToUI() {
    ...
    if (apiInput && settings.apiKey) apiinput.value = '';
    ...
    if (fb) {
        if (verified) {
            fb.textContent = '???????거?????????????븍툖??????????몃뒇?????????거???????????????濾곌쑴裕?????????????????????壤?????????????쎛 ?????????⑤슦????????????????怨룐솲??????????????????????ㅻ쑋雍? ??????????밸븶筌믩끃??獄???????멥렑??????AI ?????????????????????????????????????????????ㅻ쑋雍?';
...
function resetCodeColorSettings() {
    ...
    showToast("?????????썹땟戮녹??諭?????⑸㎦?????????????????????????怨뺤른???轅붽틓?????⑸걦??????????饔낅떽??????????????????");
}
...
function findNext() {
    ...
    if (idx !== -1) {
        ...
        showToast("?????嶺뚮씚維??????????????嶺뚮씚維??????????????븐뼐???????? ??????????롮쾸?椰????????????????ㅻ쑋雍?");
    }
}
