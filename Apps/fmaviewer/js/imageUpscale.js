...
async function applyImageTextOverlay(src, options) {
    ...
    if (!context) throw new Error("텍스트 합성을 위한 Canvas를 만들 수 없습니다.");
    context.drawImage(image, 0, 0);

    const requestedSize = Math.max(8, Math.min(500, Math.round(Number(options.size) || 48)));
    const resolutionScale = Math.max(1, Math.min(canvas.width, canvas.height) / 1000);
    const size = Math.max(8, Math.round(requestedSize * resolutionScale));
    const opacity = Math.max(.1, Math.min(1, Number(options.opacity) || 1));
    const padding = Math.max(16, Math.round(size * .55));
    const maxTextWidth = Math.max(size, canvas.width - padding * 2);
    context.font = `700 ${size}px system-ui, -apple-system, "Segoe UI", sans-serif`;
    context.lineJoin = "round";
    context.textBaseline = "top";

    const lines = wrapImageOverlayText(context, text, maxTextWidth);
    ...
    let left = padding;
    let top = padding;
    if (position.includes("right")) left = canvas.width - padding - blockWidth;
    else if (position.includes("center") || position === "center") {
        left = (canvas.width - blockWidth) / 2;
    }
    if (position.startsWith("bottom")) top = canvas.height - padding - blockHeight;
    else if (position === "center") top = (canvas.height - blockHeight) / 2;
    ...
    lines.forEach((line, index) => {
        ...
        if (position.includes("right")) x = left + blockWidth - lineWidth;
        else if (position.includes("center") || position === "center") {
            x = left + (blockWidth - lineWidth) / 2;
        }
        const y = top + index * lineHeight;
        context.strokeText(line, x, y, maxTextWidth);
...
function wrapImageOverlayText(context, text, maxWidth) {
    const lines = [];
    String(text).split(/\r?\n/).forEach(paragraph => {
        if (!paragraph) {
            lines.push("");
...
function getUpscaleTextOverlayOptions() {
    return {
        text: dom.upscaleOverlayText.value.trim(),
        position: dom.upscaleTextPosition.value,
        size: Math.max(8, Math.min(500, Number(dom.upscaleTextSize.value) || 48)),
        color: dom.upscaleTextColor.value || "#ffffff",
        opacity: (Number(dom.upscaleTextOpacity.value) || 100) / 100
    };
}

function resetUpscaleTextOverlayControls() {
    dom.upscaleOverlayText.value = "";
    dom.upscaleTextPosition.value = "bottom-center";
    dom.upscaleTextSize.value = "48";
    dom.upscaleTextColor.value = "#ffffff";
    dom.upscaleTextOpacity.value = "100";
    dom.upscaleTextOpacityValue.innerText = "100%";
}
...
function initUpscaleFeature() {
    ...
    dom.upscaleTextOpacity.oninput = () => {
        dom.upscaleTextOpacityValue.innerText = dom.upscaleTextOpacity.value + "%";
    };
...
function openUpscaleEditor(index, mode) {
    ...
    sourceImage.onload = () => {
        ...
        resetUpscaleTextOverlayControls();
...
async function runSelectedUpscale() {
    ...
    try {
        ...
        upscaleState.textOverlay = getUpscaleTextOverlayOptions();
        if (upscaleState.textOverlay.text) {
            upscaleState.resultSrc =
                await applyImageTextOverlay(upscaleState.resultSrc, upscaleState.textOverlay);
            upscaleState.resultMimeType = "image/png";
        }
        ...
        dom.upscaleDimensionInfo.innerText =
            ...
            (upscaleState.textOverlay?.text ? " · 텍스트 합성 완료" : "");
...
function saveUpscaleResult(saveMode) {
    ...
    if (saveMode === "replace") {
        ...
        sourceItem.upscaleInfo = {
            ...
            textOverlay: upscaleState.textOverlay?.text ? upscaleState.textOverlay : undefined
        };
