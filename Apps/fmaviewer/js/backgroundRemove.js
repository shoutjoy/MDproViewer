...
function initBackgroundRemoveFeature() {
    ...
    dom.bgTextOpacity.oninput = () => {
        dom.bgTextOpacityValue.innerText = dom.bgTextOpacity.value + "%";
    };
...
function openBackgroundRemoveEditor(index, mode) {
    ...
    bgRemoveState.textOverlay = null;
    ...
    resetBackgroundTextOverlayControls();
...
function getBackgroundTextOverlayOptions() {
    return {
        text: dom.bgOverlayText.value.trim(),
        position: dom.bgTextPosition.value,
        size: Math.max(8, Math.min(500, Number(dom.bgTextSize.value) || 48)),
        color: dom.bgTextColor.value || "#ffffff",
        opacity: (Number(dom.bgTextOpacity.value) || 100) / 100
    };
}

function resetBackgroundTextOverlayControls() {
    dom.bgOverlayText.value = "";
    dom.bgTextPosition.value = "bottom-center";
    dom.bgTextSize.value = "48";
    dom.bgTextColor.value = "#ffffff";
    dom.bgTextOpacity.value = "100";
    dom.bgTextOpacityValue.innerText = "100%";
}
...
async function acceptBackgroundMaskResult(resultSrc) {
    bgRemoveState.textOverlay = getBackgroundTextOverlayOptions();
    try {
        bgRemoveState.resultSrc = bgRemoveState.textOverlay.text
            ? await applyImageTextOverlay(resultSrc, bgRemoveState.textOverlay)
            : resultSrc;
    } catch (error) {
        console.error("Background text overlay error:", error);
        alert("텍스트 합성에 실패해 배경 제거 결과만 사용합니다: " + error.message);
    ...
    setBgRemoveProgress(
        100,
        bgRemoveState.textOverlay?.text ? "배경 제거 및 텍스트 합성 완료" : "배경 제거 완료"
    );
    try {
        const resultImage = await loadUpscaleImage(bgRemoveState.resultSrc);
        bgRemoveState.resultWidth = resultImage.naturalWidth;
...
function saveBackgroundRemoveResult(saveMode) {
    ...
    if (saveMode === "replace") {
        ...
        sourceItem.backgroundRemoveInfo = {
            ...
            textOverlay: bgRemoveState.textOverlay?.text ? bgRemoveState.textOverlay : undefined
        };
