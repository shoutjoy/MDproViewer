...
<div id="viewer-fs-overlay" class="viewer-fs-overlay" onclick="if(event.target.id==='viewer-fs-overlay'||event.target.id==='viewer-fs-area') viewerSSPCloseFullscreen()">
  ...
  async function viewerSSPGenerate() {
    ...
    if (!hasSeed && !prompt) { alert('????????????????????????????????ㅼ뒭???????????????????????????산뭐???????????????熬곣뫖利당춯??쎾퐲???????????????????꿔꺂?㏘틠??怨몄젦?????????????????????????????????????????????????癲??????????????????????筌????'); return; }
    var generateImage = getCallback('generateImage');
    if (typeof generateImage !== 'function') { alert('????????????????????????????????????????????????????????????????????????????????????? API?????????????????????????????????????????????嶺??'); return; }
    var statusEl = document.getElementById('ssp-status');
    var resultImg = document.getElementById('ssp-result-img');
    var downloadBtn = document.getElementById('ssp-download-btn');
    ...
    if (statusEl) statusEl.textContent = 'AI ?????????????????? ????????????諛몃마嶺뚮?????????????硫λ젒?????????????..';
    ...
    try {
      ...
      if (dataURL) {
        ...
        if (downloadBtn) downloadBtn.disabled = false;
        if (statusEl) statusEl.textContent = '????????븐뼐?????? ???????밸븶筌믩끃??????????袁⑸즴筌?씛彛??';
        viewerSSPImgHistoryAdd(dataURL, prompt);
      } else {
        if (statusEl) statusEl.textContent = '???筌왖 ??밴쉐 ??쎈솭';
      }
    } catch (e) {
      clearInterval(progressInterval);
      if (progressWrap) { progressWrap.classList.remove('visible'); progressWrap.style.display = 'none'; }
      if (statusEl) statusEl.textContent = (e && e.name === 'AbortError') ? '????????꾩룆梨띰쭕?뚢뵾???????????????????????' : ('????????????泥?? ' + (e.message || e));
    }
  }
  function viewerSSPDownload() {
    if (!__viewerSSPResultImage) { alert('?????????????????????????????????????????????????????????????????? ????????????????????????????????????嶺??'); return; }
    var a = document.createElement('a');
  ...
  async function viewerSSPUploadToImgbb(sourceDataUrl) {
    ...
    if (!sourceDataUrl || sourceDataUrl.indexOf('data:image') !== 0) {
      setSSPStatus('??????????????????????釉먮폁???????? ?????????????????????????????????????????????β뼯援?癒れ땡??????????嫄????????????????????????????????????꾩룆梨띰쭕?뚢뵾???????????????????');
      notifyUser('??????????????????????釉먮폁???????? ?????????????????????????????????????????????β뼯援?癒れ땡??????????嫄????????????????????????????????????꾩룆梨띰쭕?뚢뵾???????????????????', true);
      return;
    }
    ...
    if (!apiKey) {
      ...
      setImgbbSettingsStatus('imgBB API ??? ??????????????????????釉먮폁???????? ????????????????????????꿔꺂???⑸븶??????????????????????????⑤벡瑜????????????????????꾩룆梨띰쭕?뚢뵾???????????????????', true);
      notifyUser('imgBB API ??? ??????????????????????釉먮폁???????? ?????????????????⑤벡瑜????????????????????꾩룆梨띰쭕?뚢뵾???????????????????', true);
      return;
    }
    ...
    try {
      ...
      if (previewWindow && previewWindow.document) {
        previewWindow.document.write('<!doctype html><html><head><meta charset="utf-8"><title>imgBB ?????????/title></head><body style="font-family:Segoe UI,sans-serif;padding:24px">imgBB ?????????..</body></html>');
    ...
    try {
      ...
      setSSPStatus('imgBB ???????? ?????獄쏅챶留??????????????곸죩.');
      setImgbbSettingsStatus('????????????獄쏅챶留?? ?????獄쏅챶留??????Markdown ?????HTML ???????????壤굿??Β?????????밸쫫??????萸??', false);
      notifyUser('imgBB ???????? ?耀붾굝?????????????', false);
    } catch (e) {
